...
from simple_dnd_master import DungeonMaster
...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    try:
        ...
        print("Checking if map data exists...")
        if not dm.game_state.map_data:
            print("Generating new map...")
            await dm.generate_new_map()
        else:
            print(f"Using existing map: {dm.game_state.map_data.get('name', 'Unknown')}")
        ...
        while True:
            ...
            if message_type == "get_state":
                ...
                # Generate a new map
                new_map = await dm.generate_new_map(theme)

                # Send the new map to the client
                await websocket.send_json({
                    "type": "map_data",
                    "map": new_map,
                    "timestamp": time.time()
                ...
                # Generate a description of the new map
                state = await dm.get_screen_state()
                map_description = await dm.analyze_game_state(state)

                # Send the map description
                await websocket.send_json({
                    "type": "narration",
